...
- Uniqueness of *bom-refs* will be ensured. During ``--hierarchical`` merges, when components from a later input are added under an already present matching parent, all relocated descendants have the new parent ``bom-ref`` prepended. Existing parent prefixes are not duplicated, so relocating a subtree with ``app`` and ``app/logger`` under parent ``system`` changes those ``bom-ref`` values to ``system/app`` and ``system/app/logger``; opaque refs such as ``pkg:npm/foo@1.0`` become ``system/pkg:npm/foo@1.0``. In non-hierarchical merges, ``bom-ref`` values are left unchanged. If rebasing would create a collision, suffixes such as ``-1`` and ``-2`` are appended.
