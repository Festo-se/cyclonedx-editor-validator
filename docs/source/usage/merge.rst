...
- Uniqueness of *bom-refs* will be ensured. During ``--hierarchical`` merges, when components from a later input are added under an already present matching parent, path-style ``bom-ref`` values are rebased to the new absolute path. A ``bom-ref`` is treated as path-style only if a child's value starts with its parent's ``bom-ref`` followed by ``/``; other ``bom-ref`` formats, and non-hierarchical merges, are left unchanged. For example, relocating a subtree with ``app`` and ``app/logger`` under parent ``system`` changes those ``bom-ref`` values to ``system/app`` and ``system/app/logger``. If rebasing would create a collision, suffixes such as ``-1`` and ``-2`` are appended.
