...
class TestVex:
    ...
    def data(self, data_dir: Path, request: pytest.FixtureRequest) -> DataFixture:
        input_path = data_dir / request.param["input"]
        expected_paths = [data_dir / p for p in request.param["expected"]]

        input_vex_embedded_path = input_path

        expected_list_default = load_list(expected_paths[0])
        expected_list_lightweight = load_list(expected_paths[1])
        expected_trim_json = load_sbom(expected_paths[2])
        expected_search_json = load_sbom(expected_paths[3])
        expected_extract_json = load_sbom(expected_paths[4])

        return self.DataFixture(
            input_vex_embedded_path=input_vex_embedded_path,
            expected_list_default=expected_list_default,
    ...
    def test_list_no_state_vulnerabilities_from_embedded_file(
        ...
        # Verify that output matches what is expected
        expected_output = data["expected_list_default"]
        assert actual == expected_output

    def test_list_default_vulnerabilities_from_embedded_file(
        self,
        data: DataFixture,
        argv: Callable[..., None],
        capsys: pytest.CaptureFixture[str],
    ) -> None:
        argv("vex", "list", "--scheme", "default", str(data["input_vex_embedded_path"]))
        exit_code, actual, _ = run_main(capsys)
    ...
    def test_list_lightweight_vulnerabilities_from_embedded_file(
        self,
        data: DataFixture,
        argv: Callable[..., None],
        capsys: pytest.CaptureFixture[str],
        ...
        # Verify that output matches what is expected
        expected_output = data["expected_list_lightweight"]
        assert actual == expected_output

    def test_trim_vulnerabilities_from_embedded_file(
        self,
        data: DataFixture,
        argv: Callable[..., None],
        capsys: pytest.CaptureFixture[str],
        ...
        # Verify that output matches what is expected
        expected_output = data["expected_trim_json"]
        assert json.loads(actual) == expected_output

    def test_search_vulnerability_from_embedded_file(
        self,
        data: DataFixture,
        argv: Callable[..., None],
        capsys: pytest.CaptureFixture[str],
        ...
        # Verify that output matches what is expected
        expected_output = data["expected_search_json"]
        assert json.loads(actual) == expected_output
